...
<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-lg mx-auto pb-24">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6 relative overflow-hidden">
        ...
        <div class="grid grid-cols-7 gap-2 text-center">
            ...
            @foreach($days as $day)
                {{-- CLICKABLE CELL: Calls openDayModal() --}}
...
<script>
    ...
    const mockDailyTasks = {
        '20': [ // Green Date
            { time: '08:00 AM', title: 'Lapor Masuk', type: 'Sistem', status: 'Disahkan' },
            { time: '10:30 AM', title: 'Rondaan MPV', type: 'Rondaan', status: 'Disahkan' },
            { time: '02:00 PM', title: 'Tugas Pejabat', type: 'Admin', status: 'Disahkan' }
        ],
        '21': [ // Green Date
            { time: '08:15 AM', title: 'Lapor Masuk', type: 'Sistem', status: 'Disahkan' },
            { time: '11:00 AM', title: 'Bit / Pondok Polis', type: 'Rondaan', status: 'Disahkan' }
        ],
        '26': [ // Today
            { time: '08:00 AM', title: 'Lapor Masuk', type: 'Sistem', status: 'Menunggu' },
            { time: '10:30 AM', title: 'Rondaan Berkala', type: 'Rondaan', status: 'Selesai' }
        ]
    };
    ...
    function openDayModal(date, status, dayName) {
        ...
        if (status === 'red') {
            Swal.fire({
                icon: 'info',
                title: 'Tiada Rekod',
                text: 'Tiada log aktiviti direkodkan pada tarikh ini.',
        ...
        if (status === 'green' || status === 'today') {
            ...
            if (tasks && tasks.length > 0) {
                ...
                // Fallback if data is missing but status is green
