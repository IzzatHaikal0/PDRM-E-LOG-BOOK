...
<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-lg mx-auto pb-24">
    ...
    @if(\Illuminate\Support\Facades\Hash::check(Auth::user()->no_ic, Auth::user()->password))
        <div class="mb-6 bg-red-50 border border-red-200 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center shadow-sm relative overflow-hidden animate-fade-in">
            ...
            <a href="{{ route('Users.Profile') }}" class="z-10 shrink-0 w-full sm:w-auto text-center px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded-xl shadow-md shadow-red-600/20 transition-all active:scale-95">
                Tukar Sekarang
            </a>
        </div>
    @endif
