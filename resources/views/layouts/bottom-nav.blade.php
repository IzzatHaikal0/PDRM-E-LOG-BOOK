...
<nav class="fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
    ...
    @php 
        // 1. GET ROLE (With Fallback for Testing)
        if (Auth::check()) {
            // If strictly logged in, use database role (normalized to lowercase)
            $role = strtolower(Auth::user()->role); 
        } else {
            // --- TEST MODE: CHANGE THIS TO 'admin', 'staff', or 'officer' ---
            $role = 'admin'; 
        }

        // 2. DEBUGGING (Optional: Remove later)
        // echo ""; 
    ...
    <div class="grid h-full max-w-lg {{ $gridCols }} mx-auto font-medium">
        ...
        {{-- 1. UTAMA (Always Visible) --}}
        <a href="{{ $role === 'admin' ? route('Dashboard.Admin') : route('dashboard') }}" 
           class="{{ $baseClass }} {{ request()->routeIs('dashboard') || request()->routeIs('Dashboard.Admin') ? $activeClass : $inactiveClass }}">
            <svg class="w-6 h-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span class="text-[10px] uppercase tracking-wide font-semibold">Utama</span>
        ...
        {{-- 5. SLOT 5 (Always Visible: Profil) --}}
