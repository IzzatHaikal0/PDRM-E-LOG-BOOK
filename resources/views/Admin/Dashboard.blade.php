...
<div class="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-md transition">
            ...
            <div class="mt-2 flex items-center text-xs text-green-600">
                ...
                <span>{{ $new_penyelia_today ?? 0 }} penyelia baru hari ini</span>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="p-4 flex items-center gap-4 hover:bg-gray-50 transition">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 truncate">
                                {{ $log->description ?? $log->type ?? 'No description' }} 
                        ...
                        <span class="text-xs font-medium text-gray-400 whitespace-nowrap">
                            {{ isset($log->created_at) ? $log->created_at->diffForHumans(null, true) : '-' }}
                        </span>
                    </div>
                    @empty
        ...
        {{-- RIGHT COLUMN: Personnel Overview --}}
        <div class="space-y-4">
            <div class="flex items-center justify-between px-1">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden p-4 space-y-4">
                ...
                <button onclick='openUserModal(@json($user))' class="w-full text-left group flex items-start gap-3 p-3 rounded-xl border border-transparent hover:border-gray-100 hover:bg-gray-50 transition-all cursor-pointer">
                    {{-- Profile Image --}}
                    <div class="relative">
                        <div class="h-12 w-12 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center">
                            @if($user->gambar_profile ?? false)
...
<script>
    function openUserModal(user) {
        ...
        if (user.gambar_profile) {
            ...
            img.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || 'User')}&background=0D8ABC&color=fff`;
        }

        const pangkatName = user.pangkat ? user.pangkat.pangkat_name : 'N/A'; 
        document.getElementById('modalName').innerText = user.name || 'Unknown';
