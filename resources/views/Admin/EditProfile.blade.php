...
@section('header')
    <div class="flex items-center gap-2">
        {{-- Back Button --}}
        <a href="{{ route('Admin.Profile') }}" class="p-1 -ml-1 text-gray-400 hover:text-gray-600">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
        </a>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Kemaskini Profil Admin') }}
        </h2>
    </div>
@endsection
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('Admin.UpdateProfile', $user->id) }}" method="POST">
                ...
                <div class="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-r-lg">
                    <div class="flex gap-3">
                        ...
                        <p class="text-xs text-indigo-800 leading-relaxed font-medium">
                            Sebagai Super Admin, pastikan maklumat perhubungan anda sentiasa tepat untuk memudahkan sebarang urusan pentadbiran sistem dan pelaporan rasmi PDRM.
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-bold text-gray-700">{{ $user->no_badan ?? 'Tiada Rekod' }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">No. KP</label>
                            <p class="text-sm font-bold text-gray-700">{{ $user->no_ic ?? 'Tiada Rekod' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="pt-2 border-t border-gray-200 mt-2">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-bold text-gray-700">{{ $user->pangkat->pangkat_name ?? 'Super Admin' }}</span>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col gap-3">
                ...
                <a href="{{ route('Admin.Profile') }}" class="w-full flex justify-center items-center py-3.5 px-4 border border-gray-200 rounded-xl text-sm font-bold text-gray-600 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 transition-all active:scale-[0.98]">
                    Batal
                </a>
            </div>
